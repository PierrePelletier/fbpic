...
def test_linear_wakefield( Nm=1, show=False ):
    ...
    sim = Simulation( Nz, zmax, Nr, rmax, Nm, dt,
                      p_zmin, p_zmax, p_rmin, p_rmax, p_nz, p_nr, p_nt, n_e, zmin=zmin,
    ...
    for i in range(k_iter):
        sim.step(N_step//k_iter, correct_currents=correct_currents)
...
def compare_fields(sim, Nm, show) :
    ...
    if sim.comm.rank==0 :
        ...
        show_fields(gathered_grids[0], 'rho')
        # Analytical solution
...
# The simulation box
Nz = int(200)        # Number of gridpoints along z
...
factor = 0.8
dt = zmax/Nz/c*factor  # Timestep (seconds)
# The number of steps
N_step = int(375/factor*30 )
